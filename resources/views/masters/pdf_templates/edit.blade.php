        ...
        <div class="col-md-12">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">テンプレート編集</li>
                </ol>
            </nav>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">
                        <!-- 图标改为 Word 图标 -->
                        <i class="bi bi-file-earmark-word-fill"></i> テンプレート編集
                    </h5>
                </div>
                
                <div class="card-body">
                    <!-- 重要：添加 enctype="multipart/form-data" -->
                    <form action="{{ route('masters.pdf_templates.update', $pdfTemplate) }}" method="POST" id="templateForm" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('language_code') is-invalid @enderror" 
                                       ...
                                       value="{{ old('language_code', $pdfTemplate->language_code) }}" 
                       ...
                       <!-- ソート順 -->
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('sort') is-invalid @enderror" 
                                       id="sort" name="sort" 
                                       value="{{ old('sort', $pdfTemplate->sort) }}"
                        ...
                        <!-- 【修改点】ファイルアップロード (Word) -->
                        <div class="mb-3">
                            <label for="template_file" class="form-label">テンプレートファイル ( Word ファイルをアップロードしてください)</label>
                            <!-- 类型改为 file，移除 required (可选更新)，移除 value -->
                            <span class="d-block text-muted small fw-normal mt-1">
                                現在のファイル: <strong>{{ $pdfTemplate->template_file }}</strong>
                            </span>
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <div>
                                <a href="{{ route('masters.pdf_templates.show', $pdfTemplate) }}" class="btn btn-info text-white">
...
/* 如果需要在编辑页强调某些危险操作，可以保留这个类，虽然此页未直接使用 */
.card.border-danger {
    border-width: 2px;
}
</style>
