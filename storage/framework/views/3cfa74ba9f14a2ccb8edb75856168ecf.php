<?php $__env->startSection('title', '日別旅程一覧'); ?>
...
<div class="container-fluid px-4 py-0">
    ...
    <div class="bg-light p-2 mb-2 rounded" style="background-color: #F3F4F6 !important; border: 1px solid #E5E7EB;">
        <form method="GET" action="<?php echo e(route('masters.daily-itineraries.index')); ?>" class="row g-2">
            <div class="col-auto">
            ...
            <div class="col-auto">
                <a href="<?php echo e(route('masters.daily-itineraries.index')); ?>" class="btn btn-sm btn-outline-secondary px-3" 
    ...
    <div class="table-responsive">
        <table class="table table-sm table-bordered mb-0" style="border-color: #E5E7EB; font-size: 0.875rem;">
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $dailyItineraries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $itinerary): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr>
                    <td class="text-center px-2 py-1 align-middle">
                        <?php echo e($dailyItineraries->firstItem() + $index); ?>
                    ...
                    <td class="text-center px-2 py-1 align-middle">
                        <?php if($itinerary->time_start || $itinerary->time_end): ?>
                            <?php echo e(substr($itinerary->time_start, 0, 5) ?? '--:--'); ?>

                            〜 <?php echo e(substr($itinerary->time_end, 0, 5) ?? '--:--'); ?>

                        <?php endif; ?>
                    </td>
                    <td class="text-center px-2 py-1 align-middle">
                        <?php if($itinerary->groupInfo): ?>
                            <?php echo e($itinerary->groupInfo->agency ?? '--'); ?>

                        <?php endif; ?>
                    </td>
                    <td class="text-center px-2 py-1 align-middle">
                        <?php if($itinerary->groupInfo): ?>
                            <a href="<?php echo e(route('masters.group-infos.edit', $itinerary->groupInfo->id)); ?>" 
                               style="color: #2563eb; text-decoration: none;">
                               <?php echo e($itinerary->groupInfo->group_name ?? '--'); ?>

                            </a>
                        <?php endif; ?>
                    </td>
                    <td class="text-center px-2 py-1 align-middle">
                        <?php if($itinerary->vehicle || $itinerary->driver): ?>
                            <?php echo e($itinerary->vehicle ?? '--'); ?> / 
                            <?php echo e($itinerary->driver ?? '--'); ?>

                        <?php endif; ?>
                    </td>
                    <td class="text-center px-2 py-1 align-middle">
                        <span class="text-truncate d-inline-block" style="max-width: 120px;" title="<?php echo e($itinerary->remarks ?? ''); ?>">
                            <?php echo e($itinerary->remarks ?? '--'); ?>

                        </span>
                    </td>
                    <td class="text-center px-2 py-1 align-middle">
                        <a href="<?php echo e(route('masters.daily-itineraries.edit', $itinerary->id)); ?>" 
                           style="color: #2563eb; text-decoration: none; margin-right: 8px;">
                            編集
                        </a>
                        <a href="javascript:void(0);" onclick="confirmDelete('<?php echo e($itinerary->id); ?>', '<?php echo e($itinerary->date ? \Carbon\Carbon::parse($itinerary->date)->format('Y/m/d') : '--'); ?> の旅程')" 
    ...
    <div class="d-flex justify-content-between align-items-center mt-2">
        <div style="color: #6b7280; font-size: 0.875rem;">
            全 <?php echo e($dailyItineraries->total()); ?> 件中 
            <?php echo e($dailyItineraries->firstItem() ?? 0); ?> - <?php echo e($dailyItineraries->lastItem() ?? 0); ?> 件表示
        </div>
        <div>
            <?php echo e($dailyItineraries->withQueryString()->links('pagination::bootstrap-4')); ?>
...
function confirmDelete(id, description) {
    if (confirm(description + ' を削除してもよろしいですか？')) {
        ...
        form.action = '<?php echo e(url("masters/daily-itineraries")); ?>/' + id;
        form.submit();
    }
}
...
document.getElementById('newItineraryBtn').addEventListener('click', function() {
    openIframeModal('<?php echo e(route('masters.daily-itineraries.create')); ?>', '新規旅程作成');
});
...
document.addEventListener('DOMContentLoaded', function() {
    <?php if(request()->has('open_id')): ?>
        openDetailIframe('<?php echo e(route('masters.daily-itineraries.show', request('open_id'))); ?>');
    <?php endif; ?>
});
</script>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /www/wwwroot/default/resources/views/masters/daily-itineraries/index.blade.php ENDPATH**/ ?>